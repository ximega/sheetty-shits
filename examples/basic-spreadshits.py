...
# .cr() - is a command that sets value to a particular cell
...
sheets.cr(
    ...
    select('A-D:2-5').mixscending2D(1, xstep=2, xscend=rules.ASCEND, ystep=1, yscend=rules.DESCEND) # will increase the value of cell by 1 (or any specified value when selecting cells) in different dimensions. Can also call by just specifying step, and it will apply to both dimensions
)
